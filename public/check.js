...
function handleQuickAction(action) {
  ...
  if(action === 'Scholarships') {
    navigateTo('parents');
    response = "I've navigated you to the Scholarship Finder in the Parents section. Check the list below!";
  } else if(action === 'Top Exams') {
    navigateTo('exams');
    response = "Here is the Entrance Exam Tracker. You can filter by Engineering, Medical, and more.";
  } else if(action === '10th Options') {
    setExplorerMode('10th');
    navigateTo('explorer');
    response = "I've opened the 'After 10th' explorer. You can see Polytechnic, ITI, and other diploma paths here.";
  }
...
function handleBotResponse(query) {
  ...
  if (q.includes('college')) response = "Top colleges depend on your stream. For Engineering, IITs and NITs are premier. For Medical, AIIMS and JIPMER lead. What's your score?";
  if (q.includes('exam')) response = "Entrance exams like JEE (Engineering), NEET (Medical), and CLAT (Law) are the standard gateways. Check our 'Exams' tracker for links!";
  if (q.includes('hello') || q.includes('hi')) response = "Hello! I'm here to help you navigate your career journey. What stream are you interested in?";
